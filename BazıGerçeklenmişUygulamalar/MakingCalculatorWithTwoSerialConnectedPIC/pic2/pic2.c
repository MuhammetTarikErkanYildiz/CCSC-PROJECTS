...
char al;                                      //okunacak deðeri tutan degisken
...
{     
     ...
     if(al == '7'){
      i++;
      if(i == 1){
         printf(lcd_putc,"\f");} //i bir se sfýrla
      while(al == "7");          //Tekrar basýlana kadar bidaha döndürme
...
int islem_sec() //islem degiskenin degerini göre islem secme
{ 
delay_ms(250);
printf(lcd_putc,"\f");
   if(islem==1){
    ...
    sayi=sonuc;
    x=0;islem=0;sonuc=0;//butun degerleri sýfýrla
   }
  else if(islem==2){
    sonuc=sayi1-sayi;
    printf(lcd_putc,"%.0f",sonuc);
    ...
    sayi1=sonuc;
    sayi=sonuc;                     //sayi yedekleme sonradan sonuc üzerinden islem yapabilmek icin
    x=0;
    islem=0;
    sonuc=0;
break;
